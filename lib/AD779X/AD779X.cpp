...
unsigned char AD779X::adcCommRegByte(unsigned char registerAddressBits, unsigned char operation)
{ // choose register according the Register Address Bits and set the R/~W bit for a reading operation
...
unsigned long AD779X::adcRead(unsigned char registerSelection)
{
	...
	if (!(registerSelection == DATA_REG && adcFlag(CREAD)))
	{ // in CREAD there is no need to specify the Communication register for a read to Data register
...
void AD779X::adcWrite(unsigned char registerSelection, unsigned char val)
{ // write Mode Register and select Operating Mode OR write Offset/Full-Scale register value
	...
	else if (registerSelection == IO_REG)
	{ // write IO REGISTER
		incomingByte = SPI.transfer(val);
	}
	else if (registerSelection == OFFSET_REG || registerSelection == FULL_SCALE_REG)
	{ // write OFFSET or FULL-SCALE REGISTER (16-bits for AD7798 / 24-bits for AD7799)
...
void AD779X::adcReset()
{ // write 32 ones to reset ADC
#if DEBUG_ADC
...
void AD779X::adcResetVars()
{
	...
	_gain = 1;				// reinitialize gain
	_settleTime = 4;		// reset settle time to default value
	_numberOfChannels = 3;	// reset number of channels used to default value
	_channelIndex = 0;		// reset channel indexing
	_configRegFByte = 0x10; // default value of Configuration Register First Byte (datasheet p.16)
	_configRegSByte = 0x00; // default value of Configuration Register Second Byte (datasheet p.16)
	_modeRegFByte = 0x20;	// default value of Mode Register First Byte (datasheet p.14)
	_modeRegSByte = 0x01;	// default value of Mode Register Second Byte (datasheet p.14)
 ...
 **********************************************************************************************
 */
AD779X::AD779X(float vRef) : _spi_settings(4000000, MSBFIRST, SPI_MODE3), _vRef(vRef) {}
// {
// 	_vRef = vRef;
// }
...
void AD779X::config(unsigned char gain, unsigned char coding, unsigned char updateRate, unsigned char buffer, unsigned char refDet, unsigned char burnoutCurrent, unsigned char powerSwitch)
{
	...
	if (_adcPresent)
	{ // chip is present
		if (_modeRegSByte & 0x0F != updateRate)
		{ // check if update rate has been changed
			if (updateRate == 0x01)
			{
				_settleTime = 4;
			}
		...
		if (_configRegFByte & 0x07 != gain & 0x07)
		{ // in case the gain has been changed
			_gain = 1 << gain;
#if DEBUG_ADC
			Serial.print("Setting Gain: ");
			Serial.println(_gain);
#endif
			if (gain <= 0x02 || (gain > 0x02 && updateRate <= 0x05) | gain != 0x07)
			{							 // check if calibration is needed - datasheet p.15 - p24
				adcFlag(SET, CALIBRATE); // raise the calibration flag if needed
			}
		}
		if (adcFlag(CALIBRATE))
...
#endif
			if (timePassed >= _settleTime)
			{ // if settle time has passed
#if DEBUG_ADC
				...
				if (statusByte >> 7)
				{ // and no data available yet
#if DEBUG_ADC
					Serial.println("No data available yet.");
#endif
					if (timePassed > 4 * _settleTime)
				...
				{ // else get data, start the measurement of the next channel and reset the clock
					if (statusByte & 0x40)
					{
#if DEBUG_ADC
...
SPISettings AD779X::get_spi_settings()
{
